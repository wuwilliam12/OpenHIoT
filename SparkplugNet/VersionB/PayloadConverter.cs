...
internal static class PayloadConverter
{
    ...
    public static Metric ConvertVersionBMetric(VersionBProtoBuf.ProtoBufPayload.Metric protoMetric)
    {
        ...
        if (protoMetric.PropertySetValue != null && protoMetric.PropertySetValue.Keys != null
            && protoMetric.PropertySetValue.Values != null)
        {
            metric.Properties = new PropertySet()
            {
                Keys = protoMetric.PropertySetValue.Keys,
                Values = new List<PropertyValue>()
            };
            foreach (var v in protoMetric.PropertySetValue.Values)
                metric.Properties.Values.Add(  new  PropertyValue(DataType.String, v.StringValue) );
        }
